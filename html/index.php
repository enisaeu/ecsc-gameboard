...
END;
        $settings = "";
        $settings .= "\n" . sprintf('                                    <tr><td data-toggle="tooltip" title="CTF style">%s: </td><td><select name="style" id="%s"><option value="jeopardy"%s>Jeopardy</option><option value="ad"%s>Attack/Defense</option></select></td></tr>', "CTF style", "ctf_style", getSetting(Setting::CTF_STYLE) !== "ad" ? " selected" : "", getSetting(Setting::CTF_STYLE) === "ad" ? " selected" : "");
        ...
        $settings .= "\n" . sprintf('                                    <tr class="jeopardy"><td data-toggle="tooltip" title="Enable/disable dynamic scoring (Note: linear task cash value decrease (&alpha;=%s) based on number of solvers)">%s: </td><td><input id="%s" type="checkbox"%s></td></tr>', DYNAMIC_DECAY_PER_SOLVE, "Dynamic scoring", "dynamic_scoring", parseBool(getSetting(Setting::DYNAMIC_SCORING)) ? " checked" : "");
